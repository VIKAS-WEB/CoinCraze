...
class _CryptoWalletScreenState extends State<CryptoWalletScreen> {
  ...
  Future<void> _fetchWallets() async {
    ...
    try {
      final fetchedWallets = await apiService.getCryptoWalletBalances(); // Updated to use getCryptoWalletBalances
      print('Fetched Wallets with Balances: $fetchedWallets'); // Updated debug message
      setState(() {
            ...
            actions: [
              ...
              ElevatedButton(
                onPressed: selectedCoin == null
                    ? null
                    : () async {
                        print('Create button pressed for coin: $selectedCoin');
                        Navigator.pop(context);
                        // Map mainnet name to testnet coinName for API
                        final apiCoinName = mainnetToCoinName[selectedCoin] ?? selectedCoin!;
                    ...
                    : RefreshIndicator(
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final displayCoin = coinNameToMainnet[wallet.currency] ?? wallet.currency;
                            final imageUrl = coinImages[displayCoin] ??
                                'https://via.placeholder.com/40';
                            print('Wallet $index: coinName=${wallet.currency}, balance=${wallet.balance}'); // Updated to coinName
